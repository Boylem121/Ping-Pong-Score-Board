 $(document).ready(function() {
   ...
   $('#submit').click(function() {
    namep1 = $('#input-name-p1').val();
     $('#place-name-p1').html('<h1 class="col-xs-6 text-center">' + namep1 + '</h1>');
   });

   $('#submit').click(function() {
    namep2 = $('#input-name-p2').val();
     $('#place-name-p2').html('<h1 class="col-xs-6 text-center">' + namep2 + '</h1>');
   });

   $('#up-p1').click(function() {
       p1 = p1 + 1
       if (p1 >= winningScore && p1 >= p2 + 2) {
         $('#winner').html('<h1 class="col-xs-12 text-center"> ' + namep1 + ' wins!</h1>');
       }
       $('#score-p1').html('<h1 class="col-xs-2 text-center">' + p1 + '</h1>');
     });

   $('#up-p2').click(function() {
       p2 = p2 + 1
       if (p2 >= winningScore && p2 >= p1 + 2) {
         $('#winner').html('<h1 class="col-xs-12 text-center"> ' + namep2 + ' wins!</h1>');
       }
       $('#score-p2').html('<h1 class="col-xs-2 text-center">' + p2 + '</h1>');
     });

   $('#down-p1').click(function() {
       p1 = p1 - 1;
       return $('#score-p1').html('<h1 class="col-xs-2 text-center">' + p1 + '</h1>');
     });
   ...
   $('.score-change').click(function() {
     if ((p1 == winningScore - 1 && p1!=p2 && p2 != winningScore - 1 && p1>p2) || (p1 >= winningScore - 1 && p1>p2)) {
       $('#who-ball-p2').html('<h1 class="text-center col-xs-12"> ' + namep2 + ' is serving</h1>');
       $('#who-ball-p2').show();
       $('#who-ball-p1').hide();
     } else if ((p2 == winningScore - 1 && p2!=p1 && p1 != winningScore - 1 & p2>p1) || (p2 >= winningScore - 1 && p2>p1)) {
       $('#who-ball-p1').html('<h1 class="text-center col-xs-12"> ' + namep1 + ' is serving</h1>');
       $('#who-ball-p1').show();
       $('#who-ball-p2').hide();
